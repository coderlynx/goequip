...
//            $target = join(DIRECTORY_SEPARATOR, array($uploadDirectory, $uuid, $name));
            ...
            if ($target){
                ...
                /* Resize de la imágen */
                if (self::resizeImage($uploadDirectory.DIRECTORY_SEPARATOR.$image, $file['type'])) {
                    $result['resize success'] = true;
                }
                return $result;
            }
    ...
    private function copiarImagen($file, $destination, &$array)
    {
        ...
        $destination = $partes_ruta['dirname'].DIRECTORY_SEPARATOR.$filename;
     ...
     * @return void
     */
    private function resizeImage($filename, $type) 
    {
        if (strstr($filename, "thumb")) {
